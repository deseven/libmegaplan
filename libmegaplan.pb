...
Declare.s hmac_sha1(SecretAccessKey.s,StringToSign.s)
Declare.s getTimestamp(now.l,timezone.s)
DeclareDLL.s mega_version()
DeclareDLL.s mega_auth(login.s,password.s,base_url.s,agent.s = #ver)
DeclareDLL.s mega_query(access_id.s,secret_key.s,query.s,base_url.s,timezone.s,agent.s = #ver)
DeclareDLL.b mega_comparedates(str1.s,str2.s)

ProcedureDLL.s mega_version()
  ProcedureReturn #ver
EndProcedure
...
ProcedureDLL.s mega_auth(login.s,password.s,base_url.s,agent.s = #ver)
  ...
  post = "Login=" + login + "&Password=" + StringFingerprint(password,#PB_Cipher_MD5,0,#PB_Ascii)
  curl = curl_easy_init()
  If curl
...
Procedure.s hmac_sha1(SecretAccessKey.s,StringToSign.s)
  ...
  If(Len(SecretAccessKey) > 64)
    SecretAccessKey = StringFingerprint(SecretAccessKey,#PB_Cipher_SHA1,0,#PB_Ascii)
  ...
  *opadt = AllocateMemory(64 + 20)
  ...
  one.s = Fingerprint(*ipadt,MemorySize(*ipadt),#PB_Cipher_SHA1)
  
  For x = 64 To 84
    xx + 1
    PokeB(*opadt + x, Val("$" + Mid(one, (xx * 2) - 1, 2)))
  Next
  two.s = Fingerprint(*opadt,64+20,#PB_Cipher_SHA1)
